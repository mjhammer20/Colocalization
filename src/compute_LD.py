...
def compute_ld_for_locus(bfile: str, ld_output_dir: str, tag: str, row: pd.Series, chr_key: str, left_bound_key: str, right_bound_key: str, maf_min: float, geno_max: float) -> int:
    """
    ...
    Args:
        ...
        chr_key (str): Column name for chromosome in the loci DataFrame.
        left_bound_key (str): Column name for left boundary in the loci DataFrame.
        right_bound_key (str): Column name for right boundary in the loci DataFrame.
    ...
    rc = _run([
        ...
        "--bfile", bfile,
        "--chr", row[chr_key],
        "--from-bp", row[left_bound_key],
        "--to-bp", row[right_bound_key],
        "--maf", str(maf_min),
        "--geno", str(geno_max),
        "--keep-allele-order",
        "--make-bed",
...
def main(args: argparse.Namespace):
    ...
    for _, row in loci.iterrows():
        ...
        tag = _safe_tag(str(row[args.locus_id_key]))
        print(f"\n[{row[args.locus_id_key]}]")

        # Compute LD for the locus and get the number of variants
        n = compute_ld_for_locus(args.ref_bfile, args.ld_output_dir, tag, row, args.chr_key, args.left_bound_key, args.right_bound_key, args.maf_min, args.geno_max)

        # Append the locus information and LD file paths to the manifest
        manifest.append({
            args.locus_id_key: row[args.locus_id_key],
            "tag": tag,
            args.chr_key: row[args.chr_key],
            args.left_bound_key: row[args.left_bound_key],
            args.right_bound_key: row[args.right_bound_key],
